...
def load_model(device="cuda"):
    model = LlamaForCausalLM.from_pretrained(
        f"meta-llama/Llama-2-13b-hf", low_cpu_mem_usage=True, torch_dtype=torch.float16
    )
    model.eval()
...
def get_layer_activations_hf(
    bs: int, tokenized_prompts: torch.Tensor, attention_mask: torch.Tensor, device=None
):
    model = load_model()
    ...
    print("Starting the Dataloader loop over batches for Llama")
    for step, batch in enumerate(tqdm(dataloader, disable=False)):
        ...
        last_valid_ix = (
            torch.argmax((batch_attention_mask.sum(dim=0) > 0) * torch.arange(ctx_len)) + 1
        )
...
def write_activations_to_disk(layer_activations: dict[int, torch.Tensor], save_dir: str):
    ...
    for layer_ix, activations in layer_activations.items():
        save_path = os.path.join(save_dir, f"{layer_ix:02}.pt")
...
if __name__ == "__main__":
    main(batch_size=48, save_dir="hyper_hypo_activations1/")
